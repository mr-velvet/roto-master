...
Última atualização: 2026-05-13 madrugada — **maratona de polimento no Frames Editor.** Doze commits em sequência. Editor saiu de "MVP funcional mas com várias arestas" pra ferramenta utilizável de ponta a ponta. Mudanças por área:

- **Densidade vertical** (commit `22e363d`): discoverbar deletado (botão "prompt nesta célula" + sumário + hint — redundante com o menu de contexto). Controles de canvas (zoom/play/quadro/xadrez) migraram pro topbar central — grid de 3 colunas. Topbar e matriz colapsáveis via toggle `▼/▶` no canto sup. esq., estado em localStorage. Bug do canvas cortando imagem: `ZOOM_MIN` baixou de `1` pra `0.05`, `resetView`/`centerCanvas` agora calculam `fit-to-canvas` (`min(cw/imgW, ch/imgH) * 0.95`). Flag `userAdjustedView` preserva escolhas do user — `ResizeObserver` só refaz fit se o user ainda não interagiu com zoom/pan.

- **Modelos de IA + UX do modal de prompt** (commits `1f5ddb2`, `9676482`, `3ef761a`, `c9d49b3`, `cf7ee53`, `ce52831`, `eb8bdc3`): catálogo `FE_PROMPT_MODELS` no servidor com 3 opções — Nano Banana Pro (Gemini 3, default), Nano Banana (Gemini 2.5 Flash), GPT Image 2 (OpenAI abr/2026). `GET /api/fe/models` expõe pro front. Dropdown custom no modal. Botão "melhorar prompt" usa Claude Sonnet 4.6 com receita nova `fe-style` em `lib/prompt-recipes.js` orientada a estilização de célula (preserva pose/composição). Histórico de prompts em `localStorage` (`fe-prompt-history`, cap 50, dedup) — modal de listagem com timestamp relativo, clique reaplica texto, botão limpar. Checkbox "aplicar sobre imagem original" usa `png_url_original` como ref em vez do estado atual. Bloco informativo de aspect-ratio: `lib/providers/fal.js:planejarRatio()` calcula que ratio cada modelo entrega; badge verde "bate ✓" ou laranja "diferente — ative auto-adapt". Checkbox "auto-adapt ratio" (default on) faz letterbox preservando aspect em vez de stretch — `lib/png-resize.js` ganhou modo `'fit'` ao lado do `'stretch'`. Fix de bug: model ID do gpt-image-2 era `openai/gpt-image-2/edit` (sem prefixo `fal-ai/`), não `fal-ai/openai/...` como eu tinha posto inicialmente; timeout subiu pra 5min e quality caiu pra `medium` pra fila não dar timeout em horário de pico.

- **Undo persistido em banco** (migration `022_fe_celula_versions.sql`, commit `1f5ddb2`): tabela `fe_celula_versao` (FK CASCADE pra `fe_celula`). Antes de cada `UPDATE png_url` em `lib/fe-prompts.js`, a versão anterior vai pro histórico. `POST /api/fe/celulas/:id/undo` pop a versão mais recente (single-step, sem redo). Frontend mantém pilha `historicoPrompts` em memória (cap 50), `Ctrl/Cmd+Z` faz undo em lote pra todas as células do último disparo. Push só no sucesso. Guard ignora INPUT/TEXTAREA pra não competir com undo nativo.

- **Imagem original sagrada** (migration `023_fe_celula_png_original.sql`, commit `dad98ea`): coluna `png_url_original` em `fe_celula`. Gravada na criação da tirinha (variantes upload/asset gravam `png_url_original = png_url`). NUNCA muda depois — independente do histórico de prompts. Backfill: pra tirinhas existentes, pega a versão mais antiga em `fe_celula_versao` (ou o `png_url` atual se nunca passou por prompt). Frontend: segurar tecla `O` mostra a original no canvas. Sem feedback adicional — o próprio canvas mudando é o feedback. Célula sem original (vazia desde início): simplesmente não desenha.

- **Feedback de erro per-célula** (commit `3ef761a`): bug latente — quando uma célula falhava no provider, `fe-prompts.js` voltava ela pra `idle` com `estado_erro` preenchido, mas o frontend ignorava esse campo. Célula voltava a parecer intacta. Agora `aplicarEstadoTirinha` tira snapshot do estado anterior, detecta transição `processando → idle + estado_erro ≠ null`, dispara toast único agrupado com mensagem amigável (classifica billing / modelo não encontrado / content policy / timeout / auth). Visual: badge `!` no canto sup. dir. da célula com tooltip do erro completo. Borda rust.

- **Botão "limpar avisos"** (commit `dad98ea`): topbar ganha botão que aparece quando há células com `estado_erro`. Mostra contador. `POST /api/fe/tirinhas/:id/clear-errors` faz `UPDATE` em lote.

- **Seleção multi-célula tipo planilha** (commit `550ab7b`): Shift+click em célula define bbox entre âncora (`activeCelKey`) e clicada. Shift+click em header de quadro: range de colunas inteiras. Mesmo em header de camada. Ctrl/Cmd+click no header: toggle aditivo. Visual `.is-selected` nos headers (cyan suave + barra) quando todas as células daquela coluna/linha estão selecionadas — distinto do `.is-active` (quadro focado no canvas). Menu de contexto do quadro: quando há ≥2 quadros inteiramente selecionados, oferece "deletar N quadros" (deletes paralelos, 1 confirm, toast agregado).

- **Dimensões da matriz ajustáveis com scroll** (commit a fazer agora): wheel sobre `.fe-matrix-wrap` ajusta `matrixColWidth` (largura dos quadros) — scroll pra baixo encolhe (vê mais quadros), pra cima aumenta (preview maior). Segurando `L`, wheel ajusta `matrixRowHeight` (altura das camadas). Persistido em localStorage (`fe-matrix-col-w`, `fe-matrix-row-h`). Bounds: col 24-240px, row 32-240px, step 8px. `blur` da janela zera flag `L` pra não travar.

- **Bug de aspect ratio que distorcia imagens** (commit `cf7ee53`): modal hardcodava `aspect_ratio='1:1'` pro nano-banana e `image_size='auto'` pro gpt-image-2 → modelo cospia 1024×1024 → `resizePngTo` esticava pra widescreen → conteúdo achatado + barras pretas viravam tarjas. Backend agora calcula ratio-alvo a partir das dims da tirinha: `aspectRatioParaNano` escolhe enum mais próximo (1:1, 16:9, 9:16, 4:3, 3:4, 3:2, 2:3), `clampGptImageSize` devolve `{width, height}` custom respeitando limites do modelo (múltiplos de 16, max edge 3840, aspect ≤ 3:1).

- **Bug de SyntaxError em produção** (commit `dcdcf46`): apóstrofo dentro de string com aspas simples (`'... da' pra desfazer.'`) quebrava o parser na linha 2785. Módulo inteiro não carregava — tudo que dependia de `fe_editor.js` ficava morto. Fix trivial: removeu o apóstrofo.

**Limitações conscientes que ficaram fora desta rodada:**
- Pilha de undo do Ctrl+Z é volátil (zera ao reabrir tirinha). Histórico persiste no banco; quando virar dor, dá pra adicionar "voltar versão" no menu de contexto.
- Sem redo (`Ctrl+Shift+Z`).
- Undo só pra prompts. Criar/deletar camada/quadro/limpar célula manualmente — fora do escopo (gesto raro, dá pra recriar manualmente).
- Cancel de geração em curso continua não suportado (Fal cobra ao receber).

Última atualização anterior: 2026-05-08 madrugada — **DX endurecida: `scripts/dev.ps1` e `~/ved/devops-workflow-2026/scripts/did.ps1` reescritos pra auto-diagnóstico e auto-recovery.** Sem fricção crônica de túnel/deploy. Subcomandos canônicos `start | stop | restart | status | logs | doctor` (dev local) e `deploy | preflight | logs | tail | status | secret | ssh | doctor` (plataforma). Detalhes na seção "Fluxo de dev/deploy (2026-05-08)" abaixo.
