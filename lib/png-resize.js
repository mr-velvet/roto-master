...
// Letterbox: encaixa src dentro de (dstW, dstH) preservando aspect. Bordas
// preenchidas com alfa 0 (transparente). Resultado tem exatamente dstW x dstH.
function resizeFit(rgba, srcW, srcH, dstW, dstH) {
  const out = Buffer.alloc(dstW * dstH * 4); // alfa 0 por default
  // escala fit (min dos dois)
  ...
  for (let y = 0; y < fitH; y++) {
    ...
    for (let x = 0; x < fitW; x++) {
      ...
      const srcOff = (sy * srcW + sx) * 4;
      const dstOff = ((y + offY) * dstW + (x + offX)) * 4;
      out[dstOff] = rgba[srcOff];
      out[dstOff + 1] = rgba[srcOff + 1];
      out[dstOff + 2] = rgba[srcOff + 2];
      out[dstOff + 3] = rgba[srcOff + 3];
...
// tamanho-alvo. Modos:
//   - 'stretch' (default): nearest-neighbor esticado. Pode distorcer.
//   - 'fit': letterbox — preserva aspect, bordas transparentes.
// Se o PNG ja estiver na dimensao certa, devolve sem reencode.
...
function resizePngTo(buffer, dstW, dstH, mode = 'stretch') {
  ...
  const fn = mode === 'fit' ? resizeFit : resizeNearest;
  const resized = fn(decoded.rgba, decoded.width, decoded.height, dstW, dstH);
  const reencoded = encodeRGBA(dstW, dstH, resized);
  return { buffer: reencoded, width: dstW, height: dstH };
}

module.exports = { resizePngTo, decodeRGBA, encodeRGBA, resizeNearest, resizeFit };
