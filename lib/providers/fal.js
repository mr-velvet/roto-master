...
function bodyParaImagem(modelKey, { prompt, ref_image_urls, aspect_ratio, resolution, useEdit }) {
  ...
  // quality='medium' eh suficiente — saida volta pra resolu~cao da tirinha
  // (geralmente <=1080p) via resizePngTo, ent~ao 'high' s~o queima tempo/$
  // sem ganho visivel. 'medium' tipicamente roda em ~30-60s.
  if (modelKey === 'gpt-image-2') {
    const base = { prompt, num_images: 1, quality: 'medium', image_size: 'auto' };
