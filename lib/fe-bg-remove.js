...
async function processarCelula(pool, celulaId) {
  const { rows } = await pool.query(
    `SELECT id, tirinha_id, png_url, largura, altura
  ...
  try {
    if (!cel.png_url) throw new Error('célula sem imagem — nada pra remover fundo');

    // Chama BiRefNet via fal. Devolve URL de PNG com alpha.
    const result = await fal.removeBackground({ image_url: cel.png_url });
...
async function processarLote(pool, celulasIds) {
  console.log(`[fe-bg-remove] lote ${celulasIds.length} celulas · concorrencia=${CONCURRENCY}`);
  const fila = [...celulasIds];
  const workers = Array.from({ length: Math.min(CONCURRENCY, fila.length) }, async () => {
    while (fila.length > 0) {
      const id = fila.shift();
      if (!id) break;
      await processarCelula(pool, id);
    }
  });
