...
async function processarCelula(pool, celulaId, prompt, modelKey, usarOriginal = false, autoAdaptRatio = false) {
  ...
  try {
    ...
    await pool.query(
      ...
      [
        ...
        { usar_original: usarOriginal, auto_adapt_ratio: autoAdaptRatio },
      ]
    );
...
async function processarLote(pool, celulasIds, prompt, modelKey, usarOriginal = false, autoAdaptRatio = false) {
  const conc = concorrenciaParaModelo(modelKey);
  console.log(`[fe-prompts] lote ${celulasIds.length} celulas · model=${modelKey} · concorrencia=${conc}`);
  ...
  const workers = Array.from({ length: Math.min(conc, fila.length) }, async () => {
    while (fila.length > 0) {
      const id = fila.shift();
      if (!id) break;
      await processarCelula(pool, id, prompt, modelKey, usarOriginal, autoAdaptRatio);
    }
  });
