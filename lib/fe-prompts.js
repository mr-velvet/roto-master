...
async function processarCelula(pool, celulaId, prompt, modelKey, usarOriginal = false) {
...
async function processarLote(pool, celulasIds, prompt, modelKey, usarOriginal = false) {
  ...
  const workers = Array.from({ length: Math.min(CONCURRENCY, fila.length) }, async () => {
    while (fila.length > 0) {
      const id = fila.shift();
      if (!id) break;
      await processarCelula(pool, id, prompt, modelKey, usarOriginal);
    }
  });
